                ...
                <div class="relative text-gray-700 antialiased text-sm font-semibold">
                    ...
                    <div class="space-y-4">
                        @foreach ($ntfies as $ntfy)
                            ...
                            <div wire:key="{{ $ntfy->id }}" class="mt-6 sm:mt-0">
                                <div class="flex flex-col sm:flex-row items-center">
                                        ...
                                        <div class="w-full sm:pl-8">
                                            <div class="p-4 bg-white rounded shadow">
                                                ...
                                                <img class=" h-auto w-auto"
                                                    src="{{ Storage::url($ntfy->image) }}" alt="">

                                                @role('Admin')
                                                    <div class="inline-flex">
                                                        <x-icon-edit href="{{ route('ntfy-edit', $ntfy->id) }}" />
                                                        <x-icon-delete wire:click="delete({{ $ntfy->id }})"
                                                            class="font-medium mr-2 text-red-600 cursor-pointer"
                                    ...
                                    <div
                                        class="rounded-full bg-blue-500 border-white border-4 w-8 h-8 absolute  -translate-y-4 sm:translate-y-0 transform -translate-x-1 sm:-translate-x-1/2 flex items-center justify-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-white"
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                        </svg>
                                        <img src="image-components/Red_circle.gif" class="absolute w-44 opacity-75 {{ $ntfy->published_at->format('Y-m-d')==Carbon\Carbon::now()->format('Y-m-d') ? "" : "hidden" }}" />
                                    </div>
                                </div>
                            </div>
                        @endforeach
